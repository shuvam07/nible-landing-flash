...
const FeaturedNews = ({ articles }: FeaturedNewsProps) => {
  ...
  const heroUrl = hero.source_url || `https://nible.news/articles/${hero.unique_code}`;
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
      ...
      >
        ...
        <div className="mt-3">
          <div className="flex items-center gap-3 text-xs text-muted-foreground font-inter mb-2">
            {hero.source && (
              <span className="font-medium text-foreground/70">{hero.source}</span>
            )}
      ...
      <div className="lg:col-span-2 flex flex-col divide-y divide-border">
        {sidebar.map((article) => {
          const url = article.source_url || `https://nible.news/articles/${article.unique_code}`;
          return (
            <a
              key={article.id}
              href={url}
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between mb-1">
                  <span className="text-xs font-inter font-medium text-primary/80">
                    {article.source}
                  </span>
